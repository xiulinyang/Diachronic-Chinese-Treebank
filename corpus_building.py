...
'''Similalry, this function is to guarantee the golden corpus has at least 5000 Chinese characters.'''
...
for i, url in enumerate(lit_501):
    ...
    text = text.replace(r'<div class="page-content" id="ChapterContents">', '')
    text = text.replace(u'\xa0', u'')
...
for i, url in enumerate(lit_66):
    ...
    rep = urllib.request.urlopen(res).read().decode('utf-8', 'ignore')
    text = bf(rep, 'html.parser').find_all('p')
    text = re.sub(r'<|=|"|_|>|/|\d*|\(\);|\.|\w*书签|\+|\?|&|;|([a-zA-Z]*)|(</*p>,*)|(</*strong>)', '', str(text))
    litext_66.append(text[:-16])
...
for url in lit7808:
    ...
    text = re.sub(r'(</*br/*>)|(</*div>)|(</*p>)|(\r)|(\n)','', str(text))
    text = text.replace(r'<div class="contentbox" id="htmlContent">', '')
    text = text.replace(u'\xa0', u'')
    text = text.replace(u'\u3000', u'')
