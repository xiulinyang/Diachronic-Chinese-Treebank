...
# define a function generating all possible urls.
def getUrl(base_url, date_collection, num1, num2):
    url_collection = []
...
# ppl_collection_first = getUrl(ppl_base_url, ppl_first, 1, 7)
# print(len(ppl_collection_first))
# ppl_collection_second = getUrl(ppl_base_url, ppl_second, 1, 7)
# print(len(ppl_collection_second))
# ppl_collection_third = getUrl(ppl_base_url, ppl_third, 1,5)
# print(len(ppl_collection_third))
ppl_collection_fourth = getUrl(ppl_base_url, ppl_fourth, 1,6)
...
def getText(collection):
    ...
    return txt
#
# calculate the number of characters and text is a list
def textlen(text):
    num = 0
...
def characterlen(text):
    ...
    return num
#
# '''As there are so many links and 10 urls have 65000 characters on average,
# this program only extracts random 100 ones from People's Daily.'''
# '''Randomly choose 100 urls according to their percentage in corpus design.'''
...
# ref_collection_first = getUrl(ref_base_url, refnews_first, 1, 5)
# ref_collection_second = getUrl(ref_base_url, refnews_second, 1, 5)
# ref_collection_third = getUrl(ref_base_url, refnews_third, 1, 5)
ref_collection_fourth = getUrl(ref_base_url, refnews_fourth, 1, 16)
#
...
# # randomly select sentences containing 5000 characters.
#
...
#
# # freqdis = nltk.FreqDist(len(x) for x in a)
# print(freqdis.most_common(20))

def build_corpus(text, num1, num2):
    for i in range(len(text)):
        corpus = random.sample(text, i)
        if num1 < textlen(corpus) & textlen(corpus) < num2:
            if type(corpus) is not None:
                return corpus

def build_sample_corpus(text, num1, num2):
    for i in range(len(text)):
        corpus = random.sample(text, i)
        if num1 < characterlen(corpus) & characterlen(corpus) < num2:
            if type(corpus) is not None:
                return corpus

# # extract news from various time periods:
# corpnews_50 = build_corpus(news50, 75000, 75100)
'''政府报告字数不够了，在这里加2000'''
# corpnews_66 = build_corpus(news66, 18000, 18200)
# corpnews_78 = build_corpus(news78, 105000, 105100)
corpnews_00 = build_corpus(news00, 105000, 105100)
...
def get_work_gbk(collection):
    ...
    try:
        for i, url in enumerate(collection):
            ...
            text = re.sub(r'(</p>, <p>)|(<u>[哦一]</u>)|(</*p>)|(<br/>)|(\r)|(\n)|(\xa0)|(\u3000)|(\[)|(\[<td class="content" colspan="3">)|(<!*-*script-*>)', '', str(text))
            col.append(text)
...
def get_work(urlcollection):
    ...
    try:
        for i, url in enumerate(urlcollection):
            ...
            rep = urllib.request.urlopen(res).read().decode('utf-8', 'ignore')
            text = bf(rep, 'html.parser').find_all('p')
            text = re.sub(r'(</*p>,*)|(</*strong>)|(🍅*·鲲·弩·小·说  w w w_k u n n u_c o m)|(\u3000)|([a-zA-Z]*)|(<br>)|(\xa0)|(< ="_">< ="/6/2072\d{5})|(\d*\w*目录\w*\d*)|(\d*\w*上一章\w*\d*)', '', str(text))
            work.append(text)
...
# govrep_6 = re.split(r'(。"*）*|！"*）*|？"*|\.{6}"*）*|……"*）*|\.)', text)
# govrep_66 =[]
# for i in range(int(len(govrep_6)/2)):
#     sent = govrep_6[2 * i] + govrep_6[2 * i + 1]
#     govrep_66.append(sent)
# govrep_50 = get_work(finalurl[34:])
# govrep_50 = cut_sents(govrep_50)
# govrep_78 = cut_sents(govrep_78)
govrep_00 = cut_sents(govrep_00)
#
# print('There are ' + str(textlen(govrep_00)+textlen(govrep_50)+ textlen(govrep_66)+ textlen(govrep_78)) + ' characters in government reports.')
#
# corpgovrep_50 = build_corpus(govrep_50, 50000, 50100)
# corpgovrep_66 = build_corpus(govrep_66[5:-5], 4000, 4200)
# corpgovrep_78 = build_corpus(govrep_78, 50000, 50100)
corpgovrep_00 = build_corpus(govrep_00, 50000, 50100)
...
#     res = urllib.request.Request(url=url, headers=headers)
#     rep = urllib.request.urlopen(res).read().decode('gbk', 'ignore')
#     text = bf(rep, 'html.parser').find_all('div', id="ChapterContents")
#     text = re.sub(r'<br/>|(<div class="page-content" id="ChapterContents">)|(</*div/*>)|(\r)|(\xa0)|(\n)', '', str(text))
#     litext_501.append(text)
...
#     res = urllib.request.Request(url=url, headers=headers)
#     rep = urllib.request.urlopen(res).read().decode('gbk', 'ignore')
#     text = bf(rep, 'html.parser').find_all(class_='content')
#     text = re.sub(r'(</*br/*>)|(\r)|(\n)|(\u3000)|(\[<td class="content" colspan="3">)|(<!--script-*>)', '', str(text))
#     litext_505.append(text)
...
# print(litext_501)
# # build a corpus
# text_501 = build_corpus(litext_501[5:-5], 32150, 32250)
# text_502 = build_corpus(litext_502[5: -5], 32150, 32250)
# text_503 = build_corpus(litext_503[5: -5], 10300, 10500)
# text_504 = build_corpus(litext_504[5: -5], 4450, 4550)
# text_505 = build_corpus(litext_505[5: -5], 10300, 10400)
# text_506 = build_corpus(litext_506[5: -5], 4650, 4750)
# text_507 = build_corpus(litext_507[5: -5], 3000, 3200)
# text_508 = build_corpus(sci_50[1:-2], 3000, 3200)
# text_509 = build_corpus(litext_508[5:-5], 7000, 7100)
# text_510 = build_corpus(litext_509[5:-5], 6400, 6550)
# text_511 = build_corpus(litext_510[5:-5], 9300, 9400)
#
# print(type(text_501))
# print(type(text_502))
# print(type(text_503))
...
# corpus_50 = corpnews_50 + corpgovrep_50 + text_501 + text_502 + text_503 +text_504 + text_505 +text_506 + text_507 +text_508 + text_509 + text_510 + text_511
...
# corpus_sample_50 = build_sample_corpus(corpus_50, 1200, 1300)
# print(corpus_sample_50)
# print(textlen(corpus_sample_50))
#
# '''
# https://www.yooread.net/6/2072/ 浩然 艳阳天
# '''
# lit_66 = comp_url(63850, 63940, 'https://www.yooread.net/6/2072/', '.html')
# litext_66 = cut_sents(get_work(lit_66))
...
# sample_corpus_66 = build_sample_corpus(corpus_66, 220,280)
...
# https://www.99csw.com/book/2603/index.htm 爸爸爸
...
# https://www.99csw.com/book/2289/68659.htm 汪曾祺
...
#         text = re.sub(r'(<br/*>)|(</div>)|(<div class="zw">)|(\xa0)|(\r)|(\n)|(</*p>)|([a-zA-Z])|\[', '', str(text))
#         litext.append(text)
#     return litext
...
#     rep = urllib.request.urlopen(res).read().decode('gbk', 'ignore')
#     text = bf(rep, 'html.parser').find_all(class_ = "content")
#     text =re.sub(r'(</p>, <p>)|(<u>[哦一]</u>)|(</*p>)|(\r\n)|(</*br/*>)', '', str(text)[34:-7])
#     litext_7803.append(text)
...
#     res = urllib.request.Request(url= url, headers = headers)
#     rep = urllib.request.urlopen(res).read().decode('gbk', 'ignore')
#     text = bf(rep, 'html.parser').find_all(id ='htmlContent')
#     text = re.sub(r'(</*br/*>)|(</div>)|(<div class="contentbox" id="htmlContent">)|(</*p>)|(\r)|(\n)|(\xa0)|(\u3000)', '', str(text))
#     litext_7808.append(text)
...
# text_7803 = build_corpus(litext_7803, 14400, 14500)
# text_7804 = build_corpus(litext_7804, 14400, 14500)
# text_7805 = build_corpus(litext_7805, 17500, 17600)
# text_7806 = build_corpus(litext_7806, 17500, 17600)
# text_7807 = build_corpus(litext_7807, 5800, 5900)
# text_7808 = build_corpus(litext_7808, 8700, 8800)
# text_7809 = build_corpus(litext_7809, 8700, 8800)
# text_7810 = build_corpus(litext_7810, 14500, 14600)
# text_7811 = build_corpus(litext_7811, 6600, 6800)
# text_7812 = build_corpus(litext_7812, 6600, 6800)
# text_7813 = build_corpus(litext_7813, 14500,14600)
# text_7814 = build_corpus(litext_7814, 14500, 14600)
# text_7815 = build_corpus(litext_7815, 17500, 17600)
#
...
for i, url in enumerate(lit_0005):
    ...
    rep = urllib.request.urlopen(res).read().decode('gbk', 'ignore')
    text = bf(rep, 'html.parser').find_all('p')
    text = re.sub(r'(</*p>)|(<br/*>)', '', str(text))
    litext_0005.append(text)
...
for i, url in enumerate(lit_0006):
    ...
    try:
        res = urllib.request.Request(url= url, headers = headers)
        rep = urllib.request.urlopen(res).read().decode('utf-8', 'ignore')
        text = bf(rep, 'html.parser').find_all(id ='nr1')
        text = re.sub(r'(<div id="nr1">)|(</*p>)', '', str(text))
        litext_0006.append(text)
    except urllib.error.HTTPError:
        pass
...
text_0004 = build_corpus(litext_0004, 17500, 17600)
text_0005 = build_corpus(litext_0005, 17500, 17600)
text_0006 = build_corpus(litext_0006, 8750, 8850)
text_0007 = build_corpus(litext_0007, 4370, 4470)
text_0008 = build_corpus(litext_0008, 4370, 4470)
text_0009 = build_corpus(litext_0009, 4370, 4470)
text_0010 = build_corpus(litext_0010, 10000, 10100)
text_0011 = build_corpus(litext_0011, 4370, 4470)
text_0012 = build_corpus(litext_0012, 10000, 10100)
text_0013 = build_corpus(litext_0013, 17300, 17400)
text_0014 = build_corpus(litext_0014, 17300, 17500)
text_0015 = build_corpus(litext_0015, 17400, 17500)
...
corpus_00 = corpnews_00 + corpgovrep_00 + text_0001 + text_0002 + text_0003 \
            + text_0004 + text_0005 + text_0006 + text_0007 + text_0008 + text_0009 + \
            text_0010 + text_0011 + text_0012 + text_0013 + text_0014 + text_0015
